...
#  CONFIG  ----------------------------------------
data_folder = "saved_html"  # IRCC html
excel_file_name = "raw_data.xlsx"  # save data in excel
...
# df["Date"] = pandas.to_datetime(df["Date"]).dt.strftime("%m-%d-%Y")
# df["Date"] = pandas.to_datetime(df["Date"]).dt.strftime("%Y-%m-%d")
df["Date"] = pandas.to_datetime(df["Date"])

save_dir = os.path.join(os.path.dirname(__file__), excel_file_name)
df.to_excel(save_dir, index=False)

print(f"\nSuccess!\nCreate excel: {excel_file_name}\n")
